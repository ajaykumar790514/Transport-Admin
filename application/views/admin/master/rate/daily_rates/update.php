...
<div class="row">
          <div class="col-lg-12 col-sm-12 col-md-12">
            <div class="form-group">
				...
				<option value="">--Select--</option>
					...
					<option value="<?=$items->id;?>"  <?php if($items->id==$value->daily_item_master_id){ echo "selected";};?> ><?=$items->title;?></option>
					<?php endforeach;?>
				</select>
            </div>
        </div>
		...
		<div class="form-group">
			...
			<div class="input-group">
				<input type="number" value="<?=$value->rate;?>" class="form-control form-control" name="rate" placeholder="Enter rate">
				<select class="form-select form-select" name="rate_type">
					<option value="">--Select--</option>
					<?php foreach($min_order_qty_types as $types):?>
					<option value="<?=$types->id;?>"  <?php if($types->id==$value->rate_type){ echo "selected";};?> ><?=$types->title;?></option>
					<?php endforeach;?>
				</select>
			</div>
		</div>
